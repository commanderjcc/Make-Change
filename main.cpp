...
public:
    //Constructors
    Shop() {
        Item * NintendoSwitch = new Item(249.99, "NintendoSwitch");
        Item * Xbox = new Item(349.99,"Xbox One X");
        Item * Controller = new Item(59.99,"Wireless Controller");
        Item * Game = new Item(59.99,"Video Game");

        catalog["switch"] = *NintendoSwitch;
        catalog["xbox"] = *Xbox;
    ...
    //Methods
    bool getState() { //get method for finished
        return finished;
    };
    ...
    void goShopping() { //displays all the possible choices for the user and ask if they want to keep shopping
        ...
        cout << endl << "Enter the product ID exactly as shown or type \"checkout\" to stop shopping and check out: ";
        cin >> purchaseID;

        if (purchaseID == "checkout") { //test if they typed checkout
            finished = true; //change flag
    ...
    void checkout() { //checks out the user with cash tendered and makes them change
        ...
        justify('l', "",17, false,' ',tax,4);
        justify('r',"Total Tax | $",84);
    ...
    static void printHeader() {
        ...
        justify('c',"`---'",100,true);
        justify('l',"",100, true,'~');
        justify('c'," W E L C O M E   T O   G A M E R - M A R T ",100, true,'~');
